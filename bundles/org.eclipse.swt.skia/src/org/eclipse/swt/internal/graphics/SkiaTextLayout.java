 ...
 *
 */
public final class SkiaTextLayout {
	...
	final static Set<SkiaTextLayout> textLayouts;
	 ...
	 * @see #dispose()
	 */
	public SkiaTextLayout(Device device) {
		this.device = device;
	...
	void computeRuns(GC gc, int selectionStart, int selectionEnd,
		...
		final LineMetrics[] lineMetrics = paragraph.getLineMetrics();
		{
			// for the case of an empty string, we have to improvise a rectangle
			// with the right height.
			if (!"".equals(text)) {
				final int count = (int) paragraph.getLineNumber();
		...
		final io.github.humbleui.skija.Image im = surface
	...
	private int getLineHeight() {
		final var fm = skijaFont.getMetrics();
	...
	public void draw(GC gc, int x, int y, int selectionStart, int selectionEnd,
		...
		final int length = translateOffset(text.length());
		if (length == 0 && flags == 0) {
			return;
		}

		if (imageBytes != null) {
			final org.eclipse.swt.graphics.Image transferImage = new org.eclipse.swt.graphics.Image(
	...
	private synchronized Paragraph createParagraph(int selectionStart,
		...
		// placeholder for tab
		final var tabPlaceholder = new PlaceholderStyle(40, // Width
		...
		final boolean hasSelection = selectionStart != -1 || selectionEnd != -1;

		final ParagraphStyle style = new ParagraphStyle();
		style.setAlignment(Alignment.LEFT);

		final FontMgr fontMgr = FontMgr.getDefault();

		final FontCollection fc = new FontCollection();
		fc.setDefaultFontManager(fontMgr);

		final String fontFamily = skijaFont.getTypeface().getFamilyName();

		final io.github.humbleui.skija.paragraph.TextStyle normal = new io.github.humbleui.skija.paragraph.TextStyle()
				.setFontStyle(skijaFont.getTypeface().getFontStyle())
				.setFontSize(skijaFont.getSize())
				.setFontFamilies(new String[] { fontFamily })
				.setColor(0xFF000000);

		final io.github.humbleui.skija.paragraph.TextStyle selectionStyle = new io.github.humbleui.skija.paragraph.TextStyle()
				.setFontStyle(skijaFont.getTypeface().getFontStyle())
				.setFontSize(skijaFont.getSize())
				.setFontFamilies(new String[] { fontFamily })
				.setForeground(new Paint().setColor(SkiaGC
						.convertSWTColorToSkijaColor(selectionForeground)))
				.setBackground(new Paint().setColor(SkiaGC
		...
		try (ParagraphBuilder paragraphBuilder = new ParagraphBuilder(style,
			...
			// '\u00A0' is necessary.
			final String str = text.replace(" ", "\u00A0");
			boolean textIsStyled = false;

			for (int j = 0; j < stylesCount; j++) {
				final var si = styles[j];
				...
				final String s = str.substring(si.start, nextStyleStart);
				if (s != "") {
					if (hasSelection) {

						final var ts = convertToTextStyle(si, fontFamily);
	...
	private io.github.humbleui.skija.paragraph.TextStyle convertToTextStyle(
		...
		final TextStyle ts = si.style;

		// int foreground = SkiaGC.convertSWTColorToSkijaColor(ts.foreground);
		// int background = SkiaGC.convertSWTColorToSkijaColor(ts.background);

		if (ts == null) {

			final int foreground = SkiaGC.convertSWTColorToSkijaColor(
					device.getSystemColor(SWT.COLOR_BLACK));

			final Paint foreP = new Paint().setColor(foreground);
		...
		if (ts.background != null) {
			final int background = SkiaGC.convertSWTColorToSkijaColor(ts.background);
	...
	public Rectangle getBounds() {
		...
		if (lineBounds == null && fastCalculationMode) {
			...
			final double avgWidth = fm.getAvgCharWidth();

			final String[] splits = text.split(System.lineSeparator());

			if (splits.length == 0) {
				return new Rectangle(0, 0, 0, height);
			}

			final int linesCount = splits.length;

			final int width = 0;
			int maxChars = 0;

			final int wrapWidth = getWidth();
			double charsPerLine = 0.0;

			if (wrapWidth != -1) {
				charsPerLine = (wrapWidth) / avgWidth;
			}

			int countedLines = 0;
			for (final String s : splits) {
				final int strgLength = s.length();
				...
				if (wrapWidth != -1) {
					...
					if (maxChars <= charsPerLine) {
						...
						final double linesOfSplit = strgLength / charsPerLine;
						final int linesOfSplitInt = ((int) linesOfSplit) + 1;
		...
		for (final Rectangle r : lineBounds) {
	...
	public Rectangle getBounds(int start, int end) {
		...
		final Point startTop = getLocation(start, false);
		final Point endTop = getLocation(end, false);

		// TODO: this is only an approximation...

		final Rectangle r = new Rectangle(startTop.x, startTop.y,
	...
	public int[] getLineOffsets() {
		checkLayout();
		computeRuns(null);
		final int[] offsets = new int[lineOffsets.length];
	...
	public FontMetrics getLineMetrics(int lineIndex) {
		...
		final var lineMetricsPerLine = paragraph.getLineMetrics();
		final var lineMetrics = lineMetricsPerLine.length > lineIndex ? lineMetricsPerLine[lineIndex] : null;


		final FontMetrics fontMetrics = new FontMetricsExtension( new IExternalFontMetrics() {
			...
			public int getHeight() {
				return lineMetrics != null ? (int) lineMetrics.getHeight() : SkiaTextLayout.this.getLineHeight();
			}

			@Override
			public int getDescent() {
				return lineMetrics != null ? (int) lineMetrics.getDescent() : SkiaTextLayout.this.getDescent();
			}
			...
			@Override
			public int getAscent() {
				return lineMetrics != null ? (int) lineMetrics.getAscent() : SkiaTextLayout.this.getAscent();
			}
		});
	...
	public Point getLocation(int offset, boolean trailing) {
		...
		if (offset == text.length()) {
			final TextBox[] tbs = paragraph.getRectsForRange(offset - 1, offset,
			...
			final TextBox tb = tbs[0];

			final int x = (int) tb.getRect().getRight();
			final int y = (int) tb.getRect().getTop();

			return new Point(x, y);

		}

		final TextBox[] tbs = paragraph.getRectsForRange(offset, offset + 1,
				RectHeightMode.TIGHT, RectWidthMode.TIGHT);
	...
	public int getOffset(int x, int y, int[] trailing) {
		...
		for (final Rectangle lineBound : lineBounds) {

			if (lineBound.contains(x, y)) {
				line = lineBound;
				break;
			}
		...
		final PositionWithAffinity affinity = paragraph
	...
	public int getPreviousOffset(int offset, int movement) {
		...
		final int lineIndex = getLineIndex(offset);
		final int lineOffset = lineOffsets[lineIndex];
	...
	String getSegmentsText() {
		...
		final char defaultSeparator = orientation == SWT.RIGHT_TO_LEFT
				? RTL_MARK
						: LTR_MARK;
		while (charCount < length) {
			if (segmentCount < nSegments
					&& charCount == segments[segmentCount]) {
				final char separator = segmentsChars != null
	...
	public TextStyle getStyle(int offset) {
		...
		for (int i = 1; i < stylesCount; i++) {
			if (styles[i].start > offset) {
				...
				final TextStyle ts = styles[i - 1].style;
				return ts;
			}
		}
		return null;
	...
	boolean isUnderlineSupported(TextStyle style) {
		if (style != null && style.underline) {
			final int uStyle = style.underlineStyle;
	...
	public void setAlignment(int alignment) {
		checkLayout();
		final int mask = SWT.LEFT | SWT.CENTER | SWT.RIGHT;
		alignment &= mask;
		if (alignment == 0) {
			return;
		}
	...
	public void setOrientation(int orientation) {
		checkLayout();
		final int mask = SWT.LEFT_TO_RIGHT | SWT.RIGHT_TO_LEFT;
		orientation &= mask;
		if (orientation == 0) {
			return;
		}
	...
	public void setStyle(TextStyle style, int start, int end) {
		...
		if (0 <= high && high < stylesCount) {
			final StyleItem item = styles[high];
		...
		if (modifyStart == modifyEnd) {
			final int styleStart = styles[modifyStart].start;
			final int styleEnd = styles[modifyEnd + 1].start - 1;
			if (styleStart == start && styleEnd == end) {
				styles[modifyStart].style = style;
				return;
			}
			if (styleStart != start && styleEnd != end) {
				final int newLength = stylesCount + 2;
				if (newLength > styles.length) {
					final int newSize = Math.min(newLength + 1024,
		...
		final int newLength = stylesCount + 1 - (modifyEnd - modifyStart - 1);
		if (newLength > styles.length) {
			final int newSize = Math.min(newLength + 1024,
					Math.max(64, newLength * 2));
			final StyleItem[] newStyles = new StyleItem[newSize];
