 ...
 * </ul>
 */
public GLCanvasExtension (Canvas canvas, GLData data) {
	this.canvas = canvas;
	...
	NSNotificationCenter.defaultCenter().addObserver(canvas.view,  OS.sel_updateOpenGLContext_, OS.NSViewGlobalFrameDidChangeNotification, canvas.view);
	...
	Listener listener = event -> {
		switch (event.type) {
			...
			case SWT.Dispose:
				...
				NSNotificationCenter.defaultCenter().removeObserver(canvas.view);
...
private void checkWidget() {
	...
	if (canvas.isDisposed()) SWT.error (SWT.ERROR_WIDGET_DISPOSED);
}
}
